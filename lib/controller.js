...
var Controller = function (npmRequest, clientResponse, replicator, config) {
  ...
  function manageNewResponse(server) {
    ...
    } else if (config.hasLocalRegistry() && isLocalResponse(server) && !hasError(server)) {
      // Don't wait too long if local response is there and central registries are too long to reply
      timerCentralWaiting = setTimeout(function(){
        responseSent = true;
        responseWithAppropriatedContent('local');
      }, config.centralRegistryWait);
    }
    ...
    if (canReply() && !hasSentResponse()) {
      clearTimeout(timerCentralWaiting);
      responseSent = true;
